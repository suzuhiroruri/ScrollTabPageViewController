...
class BAScoutDetailBaseViewController: UIPageViewController {
    ...
    // 初期表示の際、スカウトメールのスクロールが行われたかどうかのフラグ
    var isMailViewScrollAdjusted = false
            ...
            self?.setViewControllers([pageViewControllers[index]],
                                     ...
                                     completion: { [weak self] (completed: Bool) in
                                        ...
                                        if shouldUpdateLayout {
                                            ...
                                            self?.setupJobDetailScrollContentOffsetY(index: index, mailViewScroll: -mailViewScrollContentOffsetY)
...
extension BAScoutDetailBaseViewController {
     ...
     */
    func setupJobDetailScrollContentOffsetY(index: Int, mailViewScroll: CGFloat) {
        guard let  jobDetailViewController = pageViewControllers[index] as? BAScoutDetailBaseViewControllerProtocol else {
            return
        }

        if mailViewScroll == 0.0 {
            // 全くスクロールしていないときはmailViewの高さをそのままjobDetailViewのOffsetに設定
            jobDetailViewController.scrollView.contentOffset.y = -mailViewHeight
        } else if mailViewScroll < (mailViewHeight - scoutDetailMailView.segmentedControlHeight.constant) ||
            ...
            jobDetailViewController.scrollView.contentOffset.y = mailViewScroll - mailViewHeight
        }
    }
    ...
    func updateJobDetailLayoutIfNeeded() {
        if shouldUpdateJobDetailContentOffset {
            ...
            // jobDetailのテーブルのスクロールのY座標をセット
            self.setupJobDetailScrollContentOffsetY(index: updateIndex, mailViewScroll: -mailViewScrollContentOffsetY)
...
extension BAScoutDetailBaseViewController: UIPageViewControllerDelegate {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, willTransitionTo pendingViewControllers: [UIViewController]) {
        if let vc = pendingViewControllers.first, let index = pageViewControllers.index(of: vc) {
            ...
            setupJobDetailScrollContentInset(index: updateIndex)
        }
    }
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        if shouldUpdateJobDetailContentOffset {
            ...
            // jobDetailのテーブルのスクロールのY座標をセット
            self.setupJobDetailScrollContentOffsetY(index: currentIndex, mailViewScroll: -mailViewScrollContentOffsetY)
        }
